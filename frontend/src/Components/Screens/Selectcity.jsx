...
export default function Selectcity() {
  ...
  useEffect(() => {
    cityselected &&
      toast.success(`${cityselected} seems a fine guess sir lets get a ride `);
  }, [cityselected]);
  ...
  return (
    <div className=" sm:w-3/4 mt-5">
      <label
        ...
        hover:bg-slate-200 animate-pulse font-bold text-shadow-black mb-3"
      >
        SELECT A City
      </label>
        ...
        {cityarr.map((city) => {
          ...
          return (
            <label
              ...
              className=" bg-red-200 relative block cursor-pointer focus:outline-none hover:scale-90 transition duration-300 ease-in-out  rounded-md"
            >
              <input
                type="radio"
                id={city}
              ...
              <div className=" w-full">
                <span className="font-bold">{city}</span> is{" "}
                {cities[city]?.Description}
                <br />
                <span className="font-bold">{city}</span> is{" "}
                {cities[city]?.Distance}Kms far away.
              </div>
            </label>
          );
        })}
      </div>
      <div className=" flex justify-center items-center mt-3 animate-pulse bg-slate-400"> 

      {cityselected && (
        <span>
          Our selected city <span className="font-bold">{cityselected}</span> is{" "}
          {cities[cityselected]?.Distance}Kms far away.
          <br />
         Vehicle with  {cities[cityselected]?.Distance * 2}Kms Range needed for a round trip .
        </span>
      )}</div>
      ...
      {cityselected && (
        <button
          className=" text-2xl mt-5 bg-black text-red-600 rounded-lg min-h-8 min-w-8 hover:text-green-600"
          onClick={() => {
            if (cityselected) {
